...
public record FluidMixingRecipe(FluidIngredient fluidInput1, FluidIngredient fluidInput2, SizedIngredient itemInput,
	...
	public boolean matches(ItemFluidContainer con, Level level) {
		...
		if (fluidInput1.isEmpty()) {
			return fluidInput2.canBeSustained(con.getFluidstack(0)) || fluidInput2.canBeSustained(con.getFluidstack(1));
		}
		
		if (fluidInput2.isEmpty()) {
			return fluidInput1.canBeSustained(con.getFluidstack(0)) || fluidInput1.canBeSustained(con.getFluidstack(1));
		}
		
		return (fluidInput1.canBeSustained(con.getFluidstack(0)) && fluidInput2.canBeSustained(con.getFluidstack(1))) ||
			(fluidInput1.canBeSustained(con.getFluidstack(1)) && fluidInput2.canBeSustained(con.getFluidstack(0)));
	}
