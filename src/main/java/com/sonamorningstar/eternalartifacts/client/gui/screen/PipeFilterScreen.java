...
public class PipeFilterScreen extends AbstractModContainerScreen<PipeFilterMenu> {
		...
		if (toConvert != null && convertingSlot instanceof FakeSlot fakeSlot) {
				...
				for (int i = 0; i < tags.size(); i++) {
					...
					innerList.addChild((x, y, width, height) -> {
							var comp = new ScrollablePanelComponent(x, y + finalI * 18, width, 16, innerList,
								(mx, my, button) -> setItemTagFilter(tag, fakeSlot.getSlotIndex()), finalI, font,
								Component.literal(tagLocation), 0xff2C2F33, 0xff3C8DBC, 0xff68C8FA);
			...
			toConvert.ifRight(fluid -> {
				...
				for (int i = 0; i < tags.size(); i++) {
					...
					innerList.addChild((x, y, width, height) -> {
							var comp = new ScrollablePanelComponent(x, y + finalI * 18, width, 16, innerList,
								(mx, my, button) -> setFluidTagFilter(tag, fakeSlot.getSlotIndex()), finalI, font,
								Component.literal(tagLocation), 0xff2C2F33, 0xff3C8DBC, 0xff68C8FA);
							comp.setRenderIcon(false);
							return comp;
						}
	...
	private void setItemTagFilter(TagKey<Item> tag, int index) {
		if (convertingSlot instanceof FakeSlot fakeSlot) {
			...
			menu.getFilters().set(index, new ItemTagEntry(tag));
			menu.getPipe().filterEntries.put(menu.getDir(), menu.getFilters());
	...
	private void setFluidTagFilter(TagKey<Fluid> tag, int index) {
		if (convertingSlot instanceof FakeSlot fakeSlot) {
			fakeSlot.set(ItemStack.EMPTY);
			menu.getFilters().set(index, new FluidTagEntry(tag));
			menu.getPipe().filterEntries.put(menu.getDir(), menu.getFilters());
	...
	protected void renderTooltip(GuiGraphics guiGraphics, int x, int y) {
		if (menu.getCarried().isEmpty() && this.hoveredSlot instanceof FilterFakeSlot fakeSlot) {
			...
			FilterEntry entry = menu.getFilters().get(fakeSlot.getSlotIndex());
			if (entry instanceof ItemTagEntry tagEntry) {
				...
				ItemStack itemStack = values[(int) ((tick / 20) % values.length)].copy();
	...
	protected void slotClicked(Slot slot, int slotId, int mouseButton, ClickType type) {
		...
		if (slot instanceof FakeSlot fakeSlot && !fakeSlot.isDisplayOnly()) {
			...
			NonNullList<FilterEntry> filters = menu.getFilters();
