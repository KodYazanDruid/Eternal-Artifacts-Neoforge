...
public class EntityFilterPanel {
	...
	public EntityFilterPanel(int x, int y, int screenWidth, int screenHeight,
		...
		panel.addChildren((fx, fy, fW, fH) -> {
			searchBox = new EditBox(font, fx + 4, fy + 17, 144, 14, Component.empty());
		...
		predicateList = new ScrollablePanel<>(panel.getX() + 4, panel.getY() + 35, 144, 108, 10);
	...
	private void buildPredicateList() {
		...
		for (EntityPredicate predicate : predicates) {
			...
			if (!searchQuery.isEmpty() && !predicate.getDisplayName().getString().toLowerCase().contains(searchQuery)) continue;
			idx++;
			int finalI = idx - 1;
			predicateList.addChild((x, y, width, height) -> {
				...
				var comp = new ScrollablePanelComponent(
					...
					(mx, my, btn, c) -> togglePredicate(predicate),
