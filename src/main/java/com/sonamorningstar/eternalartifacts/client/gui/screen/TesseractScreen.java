...
public class TesseractScreen extends AbstractModContainerScreen<TesseractMenu> {
	...
	protected void init() {
		super.init();
		addRenderableWidget(Button.builder(Component.literal("add_random"), this::addNetwork).bounds(leftPos, topPos-50, imageWidth, 25).build());
		addRenderableWidget(Button.builder(Component.literal("remove_last"), this::removeLast).bounds(leftPos, topPos-25, imageWidth, 25).build());
		addRenderableWidget(Button.builder(Component.literal("clear_selected"), this::clearSelected).bounds(leftPos, topPos-75, imageWidth, 25).build());
		setupPanel();
	...
	private void setupPanel() {
		...
		panel = new ScrollablePanel(leftPos + 10, topPos + 10 + topMargin, imageWidth - 20, imageHeight - 20 - topMargin,
	...
	public boolean mouseDragged(double mx, double my, int button, double dragX, double dragY) {
		if (panel.isFocused() &&
				isCursorInBounds(panel.getX() + panel.getWidth(),
					panel.getY(), panel.scrollbarWidth(), panel.getY() + panel.getHeight(), mx, my)) {
			panel.mouseDragged(mx, my, button, dragX, dragY);
			return true;
		}
		return false;
	}
	...
	public boolean mouseClicked(double mx, double my, int button) {
		for (AbstractWidget child : panel.getChildren()) {
			if (panel.isMouseOver(mx, my) && child.mouseClicked(mx, my + panel.scrollAmount(), button)) {
				setFocused(child);
				if (button == 0) {
					setDragging(true);
				}
