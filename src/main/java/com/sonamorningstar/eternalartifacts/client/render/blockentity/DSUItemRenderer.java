		...
		if (!stored.isEmpty()) {
			...
			for (Direction dir : Direction.values()) {
				...
				Minecraft.getInstance().getItemRenderer().render(
					...
					pose, buff, LightTexture.FULL_BRIGHT, OverlayTexture.NO_OVERLAY, Minecraft.getInstance().getItemRenderer().getModel(stored, dsu.getLevel(), null, 0)
				);
				pose.translate(-(float)2/16, -1.45 + (float)9/16, 0);
				RendererHelper.renderTextInWorld(String.valueOf(stored.getCount()), pose, buff);
				pose.popPose();
			}
