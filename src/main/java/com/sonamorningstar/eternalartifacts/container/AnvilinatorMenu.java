...
public class AnvilinatorMenu extends AbstractContainerMenu {
    public final AnvilinatorBlockEntity blockEntity;
    ...
    public AnvilinatorMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.ANVILINATOR.get(), pContainerId);
        checkContainerSize(inv, 4);
        this.blockEntity = ((AnvilinatorBlockEntity) entity);
    ...
    public int getScaledProgress() {
        ...
        int maxProgressArrowSize = 22;

        return maxProgress != 0 && progress != 0 ? progress * maxProgressArrowSize / maxProgress : 0;
    }

    public int getEnergyProgress() {
        int stored = getBlockEntity().getStoredEnergy();
        int max = getBlockEntity().getEnergyCapacity();
        int barHeight = 50;
        return max != 0 && stored !=0 ?  stored * barHeight / max : 0;
    }

    public int getFluidProgress() {
        int amount = getBlockEntity().getFluidHandler().getFluidInTank(0).getAmount();
        int max = getBlockEntity().getFluidHandler().getTankCapacity(0);
        int barHeight = 50;
    ...
    public AnvilinatorBlockEntity getBlockEntity() {
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), pPlayer, ModBlocks.ANVILINATOR.get());
    }
    ...
    public static OptionalInt openContainer(ServerPlayer player, BlockPos pos) {
        ...
        if (!(blockEntity instanceof AnvilinatorBlockEntity anvilator))
            return OptionalInt.empty();

        return player.openMenu(anvilator, pos);
    }
}
