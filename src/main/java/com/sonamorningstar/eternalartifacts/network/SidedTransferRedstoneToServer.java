...
public record SidedTransferRedstoneToServer(int index, SidedTransferMachine.RedstoneType type, BlockPos pos) implements CustomPacketPayload {
    ...
    public static SidedTransferRedstoneToServer create(FriendlyByteBuf buf) {
        return new SidedTransferRedstoneToServer(buf.readInt(), buf.readEnum(SidedTransferMachine.RedstoneType.class), buf.readBlockPos());
    }

    public static SidedTransferRedstoneToServer create(int index, SidedTransferMachine.RedstoneType type, BlockPos pos) {
    ...
    public void handle(PlayPayloadContext ctx) {
        ctx.workHandler().submitAsync(()-> ctx.player().ifPresent(player -> {
            ...
            if(entity instanceof SidedTransferMachine<?> sided) {
                Map<Integer, SidedTransferMachine.RedstoneType> redstoneConfigs = sided.getRedstoneConfigs();
