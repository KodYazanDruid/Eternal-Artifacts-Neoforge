...
public class StringUtils {
    ...
    public static List<Component> getTooltipForBlockState(@Nullable Component customName, BlockState state, @Nullable Level level, boolean isAdvanced) {
        List<Component> list = Lists.newArrayList();
		list.add(Objects.requireNonNullElseGet(customName, () -> Component.empty().append(state.getBlock().getName())));
        
        if (isAdvanced) {
            list.add(Component.literal(BuiltInRegistries.BLOCK.getKey(state.getBlock()).toString()).withStyle(ChatFormatting.DARK_GRAY));
        }
